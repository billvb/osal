...
#define MAX_SEM_VALUE               0xFFFFFFFF
...
int32 OS_API_Init(void)
{
    ...
    for(i = 0; i < OS_MAX_BIN_SEMAPHORES; i++)
    {
        ...
        OS_bin_sem_table[i].max_value     = UNINITIALIZED;
        OS_bin_sem_table[i].current_value = UNINITIALIZED;
                        ...
                        uint32 options)
{
    ...
    status = rtems_semaphore_create( r_name, sem_initial_value,0,
                                     RTEMS_BINARY_SEMAPHORE | RTEMS_NO_PRIORITY_CEILING | RTEMS_INHERIT_PRIORITY,
...
int32 OS_BinSemGive (uint32 sem_id)
{
    ...
    if ( OS_bin_sem_table[sem_id].current_value  < OS_bin_sem_table[sem_id].max_value )
    {
        ...
        {
            /* Cannot try to grab the sem when in an ISR */
