...
void OS_Application_Startup(void)
{
    ...
    printf("*********************************************************\n");
    status = TestTasks();
    printf("Number of failures in TestTasks = %d\n",status);
    totalfailures += status;
    printf("---------------------------------------------------------\n");
    
    status = TestQueues();
    printf("Number of failures in TestQueues = %d\n", status);
    totalfailures += status;
    printf("---------------------------------------------------------\n");

    status = TestBinaries();
    printf("Number of failures in TestBinaries = %d\n",status);
    totalfailures += status;
    printf("---------------------------------------------------------\n");

    status = TestMutexes();
    printf("Number of failures in TestMutexes = %d\n",status);
    totalfailures += status;
    printf("---------------------------------------------------------\n");

    status = TestGetInfos();
    printf("Number of failures in TestGetInfos = %d\n",status);
    totalfailures += status;
...
int TestQueues(void)
{
 ...
 /*     Trying now to Delete all of the Queues created. */
    ...
    status = OS_QueueDelete(msgq_0);
     /* printf("Status after Deleting q 0: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_1);
     /* printf("Status after Deleting q 1: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_2);
     /* printf("Status after Deleting q 2: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;
    ...
    status = OS_QueueDelete(msgq_4);
     /* printf("Status after Deleting q 4: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_5);
     /* printf("Status after Deleting q 5: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_6);
     /* printf("Status after Deleting q 6: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_7);
     /* printf("Status after Deleting q 7: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_8);
     /* printf("Status after Deleting q 8: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;

    status = OS_QueueDelete(msgq_9);
     /* printf("Status after Deleting q 9: %d\n",status); */
    if (status != PASS)
        failQDeletecount++;
    ...
    status = OS_QueueDelete(msgq_10);
     /* printf("Status after Deleting q 10: %d\n",status); */
    if (status == PASS)
        failQDeletecount++;
...
/*     Create Some more Queues for trying to get the id by name */
    ...
    status = OS_QueueCreate( &msgq_1, "q 0", MSGQ_DEPTH, MSGQ_SIZE, 0);
     /* printf("Status after Creating q 0 again: %d,%d\n",status,msgq_1); */
    if (status == PASS)
        failQCreatecount++;
